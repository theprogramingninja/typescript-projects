...
console.log(
	chalk.blue(
		figlet.textSync("<<< Welcome To RRS ATM >>>", {
			font: "Standard",
		...
		switch (choice.functions) {
			...
			case "Transfer Amount":
				...
				if (answers.user_balance > transfer.amount) {
					...
					console.log(
						chalk.green(`Your current balance : ${answers.user_balance}`),
					);
				} else console.log(chalk.red(`Sorry, your balance is insufficent.`));
				break;
			...
			case "Withdraw":
				...
				if (answers.user_balance > withdraw.amount) {
					...
					console.log(
						`Your balance: ${chalk.bold.green(answers.user_balance)}`,
					);
				} else console.log(chalk.red(`Sorry, your balance is insufficent.`));
				break;
